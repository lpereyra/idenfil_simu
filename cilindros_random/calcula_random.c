...
void propiedades_random(int NNN, type_real *fof)
{
  ...
  for(i=0;i<NTHREADS;i++)
  {
    ...
    #ifdef MCRITIC
    sprintf(filename,"%.2d_.4d_densidad_random_cut_%.2f_%.2f_%.2f.%.2d.bin",snap.num,NNN,m_critica,fof[0],fof[1],i);
    #else
    sprintf(filename,"%.2d_%.4d_densidad_random_%.2f_%.2f.%.2d.bin",snap.num,NNN,fof[0],fof[1],i);
    #endif
  ...
  rlong,rlong_2) shared(Segrand,PropSeg,cp,ncil,binsper,c,RMAX,RMIN,nbins)
  for(i=0;i<cp.nseg*cp.nrand;i++)
  {
    Tid = omp_get_thread_num();
    npart = (int *) calloc(ncil,sizeof(int));
    Segrand[i].Vmedia = (type_real *) calloc(3*ncil,sizeof(type_real));
    rcil2  = (type_real *) malloc(binsper*sizeof(type_real));
    l = 0;

    rsep    = PropSeg[Segrand[i].save].len/(type_real)nbins;      
    rlong   = 2.0*rsep; 
    rlong_2 = rlong*0.5;
    
    r = 0.5*PropSeg[Segrand[i].save].len;
    ...
    for(k=1;k<PropSeg[Segrand[i].save].size;k++)
    {

      r = 0.0;
      for(idim=0;idim<3;idim++)
      {
        vdir[idim] = Segrand[i].Pos[3*k+idim]-Segrand[i].Pos[3*(k-1)+idim];    
        #ifdef PERIODIC
      ...
      {
        vdir[idim] /= r;
        Pos_cent[idim] = Segrand[i].Pos[3*(k-1)+idim];
      }
      ...
      calc_media(Pos_cent,vdir,npart,Segrand[i].Vmedia,rcil2,rlong_2,ncil);             
      l++;

      r = 0.0;
      for(idim=0;idim<3;idim++)
      {
        mod_v[idim] = Segrand[i].Pos[3*k+idim]-Pos_cent[idim];
        #ifdef PERIODIC
    ...
    for(bin=0;bin<ncil;bin++)
    {
      ...
      for(idim=0;idim<3;idim++)
        Segrand[i].Vmedia[bin+ncil*idim] /= (type_real)npart[bin];
    }
  ...
  racum,dens,rsep,rlong,rlong_2) shared(pfdens,PropSeg,Segrand,cp, \
  ...
  for(i=0;i<cp.nseg*cp.nrand;i++)
  {
    ...
    fwrite(&Segrand[i].save,sizeof(int),1,pfdens[Tid]);     // escribe el padre
    fwrite(&PropSeg[Segrand[i].save].flag,sizeof(int),1,pfdens[Tid]);         // escribe la bandera
    fwrite(&PropSeg[Segrand[i].save].len,sizeof(type_real),1,pfdens[Tid]);    // escribe la longitud del segmento
    fwrite(&PropSeg[Segrand[i].save].M0,sizeof(type_real),1,pfdens[Tid]);     // escribe la masa del primer nodo
    fwrite(&PropSeg[Segrand[i].save].M1,sizeof(type_real),1,pfdens[Tid]);     // escribe la masa del ultimo nodo

    fwrite(&nbins,sizeof(int),1,pfdens[Tid]);                   // la cantidad de pelotas que hizo

    for(j=0;j<nbins;j++)
    {
      //r = nodo[j]/PropSeg[Segrand[i].save].len;      // Para normalizarlo
  ...
  assert(totfil==cp.nrand*cp.nseg);
        ...
        while(i != -1)
        {
          ...
          if(point_inside(dot,rlong_2) & 1)
          {
            ...
            //if(dis<rcil2[0])
            {
              ...
              for(k=0;k<3;k++)
              {
                Vprima[k] = P[i].Vel[k] - Segrand[icent].Vmedia[bin+binsper*k];
            ...
            for(j=0;j<binsper;j++)
            {
              if(dis<rcil2[j])
              {
                if(j==0)
                {
                  ...
                  for(k=0;k<3;k++)
                  {
                    Vprima[k] = P[i].Vel[k] - Segrand[icent].Vmedia[j+binsper*k];
...
void stadistic(int n, type_real *MAX, type_real *MIN, type_real *LMAX) 
{
  ...
  for(i=0;i<n;i++) 
  {
    a[j] = PropSeg[Segrand[i].save].len;
    mean+=a[j];
    sumquad+=a[j]*a[j];
    j++;
  }
