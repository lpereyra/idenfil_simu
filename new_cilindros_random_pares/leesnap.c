...
extern void read_pares(type_int NNN, type_real *fof)
{
  ...
  #ifdef MCRITIC
    sprintf(filename,"../new_pares_random/%.2d_%.4d_pares_halos_%.2f_%.2f_%.2f.bin",snap.num,NNN,m_critica,fof[0],fof[1]);
  #else
    sprintf(filename,"../new_pares_random/%.2d_%.4d_pares_halos_%.2f_%.2f.bin",snap.num,NNN,fof[0],fof[1]);
  #endif
