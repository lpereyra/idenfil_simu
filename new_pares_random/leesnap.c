...
static void set_name(const type_int flag_smooth, const char * prefix, char * name, const type_int NNN, const type_real * fof)
{
...
extern void read_segment(type_int NNN, type_real *fof)
{
  ...
  char  filename[200];
  FILE  *pf;
  type_int flag_smooth;

  flag_smooth = 0;
  set_name(flag_smooth,"segmentos",filename,NNN,fof);
  fprintf(stdout,"%s\n",filename);
  ...
  for(i=0;i<cp.nseg;i++)
  {
    ...
    fread(&Seg[i].list[0],sizeof(type_int),1,pf);
    fseek(pf,k*sizeof(type_int),SEEK_CUR);
    fread(&Seg[i].list[1],sizeof(type_int),1,pf);
  ...
  fclose(pf);
    ...
    flag_smooth = 1;
  #endif
  set_name(flag_smooth,"propiedades",filename,NNN,fof);
  fprintf(stdout,"%s\n",filename);
...
extern void read_grup_fof(type_real *fof)
{
  ...
  #ifdef MCRITIC
    sprintf(filename,"../../halos_propiedades/%.2d_%.2f_centros_propiedades_cut_%.2f.bin",snap.num,fof[1],m_critica);
  #else
    sprintf(filename,"../../halos_propiedades/%.2d_%.2f_centros_propiedades.bin",snap.num,fof[1]);
  #endif
  pfin = fopen(filename,"rb"); 
