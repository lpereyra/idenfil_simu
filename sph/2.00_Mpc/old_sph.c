...
extern void init_sph(const type_int i)
{
  ...
  // Saco factor comun el 12 Pi para restar los volumenes de las intersecciones
 
  #pragma omp parallel for num_threads(NTHREADS) \
  schedule(dynamic) default(none) private(j,k,r,d,diff) \
  shared(cp,Seg,pmin,stdout) reduction(+:vol)
  for(k=0;k<Seg[i].size;k++)
  {
    ...
    for(j=0;j<3;j++)
    {
      r[j] = Seg[i].Pos[3*k+j]-Seg[i].Pos[j];
      if(r[j]> 0.5*cp.simulbox) r[j] -= cp.simulbox;
      if(r[j]<-0.5*cp.simulbox) r[j] += cp.simulbox;
      d += r[j]*r[j];
    }
    ...
    if(d<(Seg[i].Rvir[0]+HSPH))
    {
      if(d>fabs(Seg[i].Rvir[0]-HSPH)){ // Conditions for intersection
        vol += pow((Seg[i].Rvir[0]+HSPH-d),2.0f) * \
              (d+2.0f*(Seg[i].Rvir[0]+HSPH)  - \
              (3.0f/d)*(h2+Seg[i].Rvir[0]*Seg[i].Rvir[0]) + \
              (6.0f/d)*(Seg[i].Rvir[0]*HSPH));
        diff = 1;
      diff = -1;
      }else if (Seg[i].Rvir[0]<HSPH){  // else one sphere inside the other
        vol += 16.0f*pow(Seg[i].Rvir[0],3.0f);
        diff = 1;
      }else{
        vol += 16.0f*h2*HSPH;
        diff = -1;
    ...
    for(j=0;j<3;j++)
    {
      r[j] = Seg[i].Pos[3*k+j]-Seg[i].Pos[3*(Seg[i].size-1)+j];
      if(r[j]> 0.5*cp.simulbox) r[j] -= cp.simulbox;
      if(r[j]<-0.5*cp.simulbox) r[j] += cp.simulbox;
      d += r[j]*r[j];
    }
    ...
    if(d<(Seg[i].Rvir[1]+HSPH))
    {
      if(d>fabs(Seg[i].Rvir[1]-HSPH)){ // Conditions for intersection
        vol += pow((Seg[i].Rvir[1]+HSPH-d),2.0f) * \
              (d+2.0f*(Seg[i].Rvir[1]+HSPH)  - \
              (3.0f/d)*(h2+Seg[i].Rvir[1]*Seg[i].Rvir[1]) + \
              (6.0f/d)*(Seg[i].Rvir[1]*HSPH));
        diff = Seg[i].size;
      }else if (Seg[i].Rvir[1]<HSPH){  // else one sphere inside the other
        vol += 16.0f*pow(Seg[i].Rvir[1],3.0f);
        diff = Seg[i].size;
      }else {
        vol += 16.0f*h2*HSPH;
        diff = -Seg[i].size;
    ...
    if(k!=(Seg[i].size-1) && diff>=0)
    {
      d = 0.0f;
      for(j=0;j<3;j++)
      {
        r[j] = Seg[i].Pos[3*(k+1)+j]-Seg[i].Pos[3*k+j];
        if(r[j]> 0.5*cp.simulbox) r[j] -= cp.simulbox;
        if(r[j]<-0.5*cp.simulbox) r[j] += cp.simulbox;
        d += r[j]*r[j];
      }
      ...
      if(diff!=0)
      {
        ...
        for(j=0;j<3;j++)
        {
          r[j] = Seg[i].Pos[3*(k+1)+j]-Seg[i].Pos[3*diff+j];
          if(r[j]> 0.5*cp.simulbox) r[j] -= cp.simulbox;
          if(r[j]<-0.5*cp.simulbox) r[j] += cp.simulbox;
          d += r[j]*r[j];
        }
        ...
        diff = (int)(Seg[i].size-1) == diff ? 1 : diff;

        if(d<(Seg[i].Rvir[diff]+HSPH) && d>fabs(Seg[i].Rvir[diff]-HSPH))
        { // Conditions for intersection correction factor
              vol -= pow((Seg[i].Rvir[diff]+HSPH-d),2.0f) * \
              (d+2.0f*(Seg[i].Rvir[diff]+HSPH)  - \
              (3.0f/d)*(h2+Seg[i].Rvir[diff]*Seg[i].Rvir[diff]) + \
              (6.0f/d)*(Seg[i].Rvir[diff]*HSPH));
        }
      } 
    }
    */
  ...
  vol  = Seg[i].Rvir[0]*Seg[i].Rvir[0]*Seg[i].Rvir[0]+Seg[i].Rvir[1]*Seg[i].Rvir[1]*Seg[i].Rvir[1];
  vol *= (4.0/3.0);
  vol *= M_PI;
  vol = (float)vvv - vol;
  ...
  //vol = Seg[i].size*16.0f*h2*HSPH - vol;
  //vol *= ( M_PI/12.0f ); // factor comun Pi/12.0
